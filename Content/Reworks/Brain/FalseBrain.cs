...
using BigEvil.Common.Netcode.Packets;
...
public class FalseBrain : ModNPC, ILocalizedModType
{
    ...
    public override void AI()
    {
        ...
        {
            float lerp = MathUtils.SineInOutEasing(MathHelper.Clamp(SpawnTime / -30f, 0f, 1f));
            float baseDist = 240;
    ...
    public override void OnHitByItem(Player player, Item item, NPC.HitInfo hit, int damageDone)
    {
        if (!BeenHit)
        {
            if (Main.netMode == NetmodeID.SinglePlayer)
                ...
                BigEvilNet.SendNetMessage<BrainIllusionHitPacket>(NPC.whoAmI, player.whoAmI);
            NPC.dontTakeDamage = true;
        }
    }
    ...
    {
        if (!BeenHit && !projectile.GetGlobalProjectile<BrainOfCthulhuGlobalProjectile>().IgnoreBoCIllusions && projectile.owner != -1)
        {
            if(Main.netMode == NetmodeID.SinglePlayer)
                ...
                BigEvilNet.SendNetMessage<BrainIllusionHitPacket>(NPC.whoAmI, projectile.owner);
            NPC.dontTakeDamage = true;
        }
    }
