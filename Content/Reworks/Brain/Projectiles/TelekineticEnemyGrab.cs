...
public class TelekineticEnemyGrab : ModProjectile, ILocalizedModType
{
    ...
    public override string Texture => "BigEvil/Common/Graphics/Particles/Types/BloomRing";
    ...
    public override void AI()
    {
        ...
        if (!thrown)
        {
            ...
            if (StunTime == -1)
            {
                ...
                else
                {
                    ...
                    if (throwTime >= 30 && throwTime <= 90)
                    {
                        if (throwTime <= 60f)
                        {
                            Projectile.Center = Vector2.Lerp(throwPos, throwPos - throwDir * 56f, MathUtils.SineInOutEasing((throwTime - 30) / 30f)) + (Main.npc[NPC.crimsonBoss].Center + Main.npc[NPC.crimsonBoss].velocity);
            ...
            float lerp = MathUtils.SineInOutEasing(MathHelper.Clamp((throwTime) / 60f, 0f, 1f));
