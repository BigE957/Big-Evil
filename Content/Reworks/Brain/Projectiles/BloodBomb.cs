...
public class BloodBomb : ModNPC, ILocalizedModType
{
    ...
    public override void SetStaticDefaults()
    {
        ...
        BloodBombYellow = ModContent.Request<Texture2D>(Texture + "2");
    ...
    public override void OnSpawn(IEntitySource source)
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            BloodDrop p = new(NPC.Center, NPC.velocity.RotatedBy(Main.rand.NextFloat(-MathHelper.Pi / 6f, MathHelper.Pi / 6f)) * Main.rand.NextFloat(2f, 3f), 32, 1f, Color.Red);
            ParticleSystem.SpawnParticle(p);
        }
        BloodSplatter p2 = new(NPC.Center, NPC.velocity * 2.5f, 16, 0.5f, Color.Red);
    ...
    public override void AI()
    {
        ...
        if (NPC.ai[0] % 3 == 0)
        {
            ...
            BloodDrop p = new(NPC.Center + Main.rand.NextVector2Circular(NPC.width, NPC.height), (-NPC.velocity).RotatedBy(Main.rand.NextFloat(-MathHelper.Pi / 6f, MathHelper.Pi / 6f)) * Main.rand.NextFloat(0.25f, 0.75f), Main.rand.Next(10, 17), Main.rand.NextFloat(0.5f, 1f), color);
            ParticleSystem.SpawnParticle(p);
        }
    ...
    {
        if (projectile.owner != -1 && modifiers.DamageType.CountsAsClass(DamageClass.Melee) && Main.player[projectile.owner].heldProj == projectile.whoAmI)
        {
            modifiers.FinalDamage *= 1.5f;
            return;
        }
...
public class BloodBombRTS : ModProjectile, ILocalizedModType
{
    ...
    public override string Texture => ModContent.GetInstance<BloodBomb>().Texture;
    public override void SetStaticDefaults()
    {
    ...
    public override void OnKill(int timeLeft)
    {
        ...
        CustomPulse explosion1 = new(Projectile.Center, Vector2.Zero, Color.OrangeRed, "BigEvil/Common/Graphics/Particles/Types/FlameExplosion", Vector2.One, Main.rand.NextFloat(0f, MathHelper.Pi), 0f, 0.1f, 24);
        ParticleSystem.SpawnParticle(explosion1);

        CustomPulse explosion2 = new(Projectile.Center, Vector2.Zero, Color.Red, "BigEvil/Common/Graphics/Particles/Types/FlameExplosion", Vector2.One, Main.rand.NextFloat(0f, MathHelper.Pi), 0f, 0.075f, 24);
        ParticleSystem.SpawnParticle(explosion2);
    }
