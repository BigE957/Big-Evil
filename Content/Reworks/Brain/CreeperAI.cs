...
public class CreeperAI : VanillaAIOverride
{
    ...
    public override bool AI(Mod mod)
    {
        ...
        if (NPC.target < 0 || NPC.target == Main.maxPlayers || Main.player[NPC.target].dead || !Main.player[NPC.target].active)
            NPCUtils.EnhancedTargeting(NPC, default);
        #endregion
    ...
    private void TelekineticOnslaught()
    {
        if (Time > 0)
        {
            ...
            {
                CollisionUtils.RayCast(brain.Center, AttackAngle.ToRotationVector2(), 800, out float rayDist); //Step 4
    ...
    private void CreeperSwipes()
    {
        ...
        else
        {
            ...
            else
            {
                ...
                goalLocation.Y += MathHelper.Lerp(0, 420, MathUtils.SineInOutEasing(MathHelper.Clamp((Time - brain.AIOverride<BrainOfCthulhuAI>().SwipeDelay) / 20f, 0f, 1f)));

                if (!singleHand)
                    goalLocation.X += MathHelper.Lerp(0f, 900f, MathUtils.SineInOutEasing(MathHelper.Clamp((Time - (brain.AIOverride<BrainOfCthulhuAI>().SwipeDelay + 5)) / 55f, 0f, 1f))) * (evenID ? 1 : -1);
                else
                    goalLocation.X += MathHelper.Lerp(0f, 900f, MathUtils.SineInOutEasing(MathHelper.Clamp((Time - (brain.AIOverride<BrainOfCthulhuAI>().SwipeDelay + 5)) / 55f, 0f, 1f))) * (handSide == -1 ? 1 : -1);
        ...
        else
        {
            ...
            if (!bocAI.OnSecondCreeperPhase) // Swipe Attack
            {
                ...
                if (Time <= positioningTime)
                {
                    ...
                    NPC.Center = Vector2.Lerp(AttackPosition, goalPosition, MathUtils.SineOutEasing(MathHelper.Clamp(Time / (float)LightSwipeTravelTime, 0f, 1f)));
            ...
            else //Crush Attack
            {
                ...
                if (Time <= positioningTime)
                {
                    if (AttackPosition == Vector2.Zero)
                        AttackPosition = NPC.Center;
                    Vector2 goalPosition = sharedTarget.Center - dashDir * 128;
                    NPC.Center = Vector2.Lerp(AttackPosition, goalPosition, MathUtils.SineOutEasing(MathHelper.Clamp(Time / (float)StrongSwipeTravelTime, 0f, 1f)));
                    NPC.velocity = Vector2.Zero;
                    DisableMultiplayerSmoothing = true;
                }
                else
                        ...
                        if (Main.npc[PartnerIndex].active)
                        {
                            ...
                            if (Time == (positioningTime + reelbackTime + swingTime))
                            {
                                ...
                                if (dashDir.X <= 0)
                                {
                                    ...
                                    CustomPulse splatter = new(attackCenter, Vector2.Zero, Color.Red, "BigEvil/Common/Graphics/Particles/Types/SmokeExplosion", Vector2.One, Main.rand.NextFloatDirection(), 0.05f, 0.15f, 24);
                                    ParticleSystem.SpawnParticle(splatter);
                                }
    ...
    private void CreeperOrbit()
    {
        ...
        if (Time >= 0)
        {
            ...
            if (Time < telegraphPeriod)
                dist = MathHelper.Lerp(dist, OrbitTelegraphRadius, MathUtils.SineOutEasing(Time / telegraphPeriod));
            else if (Time < shiftPeriod + telegraphPeriod)
                dist = MathHelper.Lerp(OrbitTelegraphRadius, 16, MathUtils.SineInOutEasing((Time - telegraphPeriod) / shiftPeriod));
            else
            {
                dist = MathHelper.Lerp(16, dist, MathUtils.SineInOutEasing((Time - telegraphPeriod - shiftPeriod) / shiftPeriod));
        ...
        {
            NPC.damage = 0;
            NPC.Center = Vector2.Lerp(AttackPosition, bocAI.AttackPosition + rotation, MathUtils.SineOutEasing(bossCounter / OrbitSetupDuration));
        }
        else
        {
            NPC.damage = NPC.defDamage;
    ...
    private void CreeperSpiral()
    {
        ...
        else
        {
            ...
            NPC.Center = Vector2.Lerp(AttackPosition, position, MathUtils.SineInOutEasing(MathHelper.Clamp(lerp, 0f, 1f)));
            DisableMultiplayerSmoothing = true;
        }
    ...
    public override void HitEffect(Mod mod, NPC.HitInfo hit)
    {
        ...
        {
            VerletObject verletTendril = BrainOfCthulhuSystem.VerletTendrils[CreeperID].tendril;
            ...
            for (int i = 0; i < 5; i++)
            {
                ...
                BloodDrop p = new(verletTendril[^2].Position, dir.RotatedBy(Main.rand.NextFloat(-MathHelper.Pi / 10f, MathHelper.Pi / 10f)) * Main.rand.NextFloat(6f, 10f), 24, Main.rand.NextFloat(0.5f, 1f), Color.Yellow * 0.75f);
