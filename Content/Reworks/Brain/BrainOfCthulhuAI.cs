...
using static BigEvil.Common.Utilities.NPCUtils;

namespace BigEvil.Content.Reworks.Brain;

public class BrainOfCthulhuAI : VanillaAIOverride
{
    private static SoundStyle StunnedHit = new SoundStyle("BigEvil/Content/Reworks/Brain/Sounds/BoC_Rev_Stun_Hit", 3);
    private static SoundStyle ShieldDown = new SoundStyle("BigEvil/Content/Reworks/Brain/Sounds/BoC_Rev_Shield_Down") with { PauseBehavior = PauseBehavior.PauseWithGame };
    private static SoundStyle ShieldUp = new SoundStyle("BigEvil/Content/Reworks/Brain/Sounds/BoC_Rev_Shield_Up") with { PauseBehavior = PauseBehavior.PauseWithGame };
    private static SoundStyle IntroRoar = new SoundStyle("BigEvil/Content/Reworks/Brain/Sounds/BoC_Rev_Roar") with { PauseBehavior = PauseBehavior.PauseWithGame };
    private static SoundStyle Roar = new SoundStyle("BigEvil/Content/Reworks/Brain/Sounds/BoC_Rev_Short_Roar") with { PauseBehavior =  PauseBehavior.PauseWithGame};
    public static SoundStyle Laugh = new SoundStyle("BigEvil/Content/Reworks/Brain/Sounds/BoC_Rev_Laugh") with { PauseBehavior = PauseBehavior.PauseWithGame, MaxInstances = 5 };
    private static SoundStyle Growl = new SoundStyle("BigEvil/Content/Reworks/Brain/Sounds/BoC_Rev_Growl", 2) with { PauseBehavior = PauseBehavior.PauseWithGame };
    private static SoundStyle Death = new SoundStyle("BigEvil/Content/Reworks/Brain/Sounds/BoC_Rev_Death_Roar") with { PauseBehavior = PauseBehavior.PauseWithGame };
    private static SoundStyle BloodShot = new SoundStyle("BigEvil/Content/Reworks/Brain/Sounds/BoC_Rev_BloodShot");
    private static SoundStyle BloodBomb = new SoundStyle("BigEvil/Content/Reworks/Brain/Sounds/BoC_Rev_BloodBomb");
    private static SoundStyle BloodExplosion = new SoundStyle("BigEvil/Content/Reworks/Brain/Sounds/BoC_Rev_Explosion", 2);
    ...
    internal int SwipeDelay => AttackFlag ? 30 : Main.masterMode ? 40 : 50;
    ...
    internal static int LightSwipeAmount => Main.masterMode ? 8 : 6;
    internal static int LightSwipeTravelTime => 30;
    internal static int LightSwipeAttackDelay => 10;
    internal static int LightSwipeDuration => Main.masterMode ? 45 : 60;
    internal static int StrongSwipeDelay => 90;
    internal static int StrongSwipeAmount => Main.masterMode ? 7 : 5;
    internal static int StrongSwipeTravelTime => 45;
    internal static int StrongSwipeAttackDelay => 15;
    internal static int StrongSwipeDuration => Main.masterMode ? 60 : 80;
    #endregion
    ...
    internal static int OrbitAttackParticipantCount => Main.masterMode ? 4 : 3;
    ...
    internal static int StartingTimePerRevolutionMax => Main.masterMode ? 270 : 300;
    internal static int StartingTimePerRevolutionMin => Main.masterMode ? 180 : 210;
    internal static int EndingTimePerRevolutionMax => Main.masterMode ? 210 : 240;
    internal static int EndingTimePerRevolutionMin => Main.masterMode ? 120 : 150;
    ...
    internal static float TurnAroundDurationRatio => Main.masterMode ? 0.1f : 0.125f; //The amount of time that it'll take for the creeper spiral to turn around
    ...
    internal static int IdleTeleportDuration => Main.masterMode ? 36 : 44;
    internal static float ChaseMinSpeed => 3;
    internal static float ChaseMaxSpeed => Main.masterMode ? 18 : 15;
    ...
    internal static int IchorRate => Main.masterMode ? 10 : 12;
    ...
    internal static int DashScytheRate => Main.masterMode ? 5 : 6;
    #endregion

    #region Sanguine Scythes
    internal static int SanguineTeleportCount => 5;
    internal static int SanguineScytheCount => Main.masterMode ? 12 : 10;
    ...
    internal static int IllusionTrickAngleGroups => Main.masterMode ? 8 : 6;
    internal static int IllusionTrickGroupSize => Main.masterMode ? 5 : 4;
    ...
    internal bool WasSummonedViaItem = false;
    ...
    private static float CreeperHPRatio { get
        {
            ...
            if (ratio != 0f)
                ratio /= GetBrainOfCthuluCreepersCount();
            return ratio;
        }
    }

    private static float CreeperAmountRatio => NPC.CountNPCS(NPCID.Creeper) / (float)GetBrainOfCthuluCreepersCount();
    ...
    public override void SetDefaults(Mod mod)
    {
        ...
        if (!Main.dedServ)
        {
            ...
            BrainOfCthulhuSystem.VerletTendrils = new (int creeper, VerletObject tendril, int reelInTimer)[brainOfCthuluCreepersCount];

            for (int i = 0; i < brainOfCthuluCreepersCount; i++)
                BrainOfCthulhuSystem.VerletTendrils[i].tendril = VerletIntegration.CreateVerletChain(NPC.Center, NPC.Center, 28, 16);
        }
    }
    ...
    public override void OnSpawn(Mod mod)
    {
        if (NPC.target < 0 || NPC.target == Main.maxPlayers || Main.player[NPC.target].dead || !Main.player[NPC.target].active)
        {
            ...
            TargetingParameters options = TargetingParameters.BossDefaults;
            options.aggroRatio = -1f;
            options.finishThemOff = true;
            NPCUtils.EnhancedTargeting(NPC, options);
        }
    ...
    private void SpawnAnimation()
    {
        NPC.damage = 0;
        /*
        Main.NewText("Time: " + Time);
        Main.NewText("SpawnTime: " + SpawnTime);
        Main.NewText("AttackCounter: " + AttackCounter);
        */
        if (SpawnTime != 0) //BoC should begin appearing
        {
            ...
            float spawnAnimTimeReduction = WasSummonedViaItem  ? 120 : 0;
            float spawnCounter = Time - Math.Abs(SpawnTime) + spawnAnimTimeReduction;

            if (spawnCounter < 180)
            {
                float shakeIntensity = MathUtils.CircOutEasing(spawnCounter / 180f) * 3f * distanceScaleFactor;
                CameraSystem.StartScreenShake(NPC.Center, Vector2.Zero, shakeIntensity, 10, 60);
            ...
            else if (spawnCounter > 180)
            {
                ...
                if (spawnCounter > 240 && spawnCounter < 390)
                {
                    ...
                    CameraSystem.StartScreenShake(NPC.Center, Vector2.Zero, 6 * BrainOfCthulhuSystem.ScreenBlurStrength * distanceScaleFactor, 10, 60);
        ...
        if (AttackCounter < GetBrainOfCthuluCreepersCount())
        {
            if (SpawnTime == 0)
            {
        ...
        else if (SpawnTime == 0 && Main.netMode != NetmodeID.MultiplayerClient)
        {
            SpawnTime = Time;
            NPC.netUpdate = true;
        }
    ...
    private void Phase1Idle()
    {
        ...
        */
        ...
        Vector2 fromTarget = Target.DirectionTo(NPC.Center);
        Vector2 dir = fromTarget.RotatedBy(Math.Sin(Time / 60f) * rotateDir) * new Vector2(2, 1);
        Vector2? hitspot = CollisionUtils.RayCast(Target.Center, dir, 360, out float rayDist);
        Vector2 goalPos = hitspot ?? Target.Center + dir * 360;
    ...
    private void TelekineticOnslaught()
    {
        ...
        if (Time <= 60)
        {
            if (Time == 0)
            {
                ...
                if (Main.netMode != NetmodeID.MultiplayerClient)
                {
                    foreach (NPC creeper in Main.ActiveNPCs)
                    {
                        ...
                        Vector2? hitspot = CollisionUtils.RayCast(NPC.Center, ai.AttackAngle.ToRotationVector2(), 800, out float rayDist); //Step 4
            ...
            if (Time < 30)
            {
                ...
                CameraSystem.StartScreenShake(NPC.Center, Vector2.Zero, BrainOfCthulhuSystem.ScreenBlurStrength * distanceScaleFactor, 10, 60);
                ...
                BrainOfCthulhuSystem.ScreenBlurStrength = MathHelper.Lerp(0.5f, 0, MathUtils.CircOutEasing((Time - 30) / 30f));
        }
        else
        {
            BrainOfCthulhuSystem.ScreenBlurStrength = 0f;
            if (wrappedCounter == 65)
            {
                ...
                CollisionUtils.RayCast(NPC.Center, Vector2.UnitX * AttackSign, 480f + NPC.width, out float wallDist);
                wallDist -= NPC.width;
                ...
                for (int i = 0; i < checkCount; i++)
                {
                    ...
                    Vector2 startPos = NPC.Center + (Vector2.UnitX * ((wallDist * completion) + NPC.width) * AttackSign);

                    CollisionUtils.RayCast(startPos, Vector2.UnitY, maxDist, out float floorDist);

                    Vector2 endPos = startPos + (Vector2.UnitY * (floorDist + 48f));
                    ...
                    if (floorDist >= 600f)
                        ...
                        closeEnds.Add(endPos);
                }

                Vector2 chosenEnd = NPC.Center;
    ...
    private void Stunned()
    {
        ...
        if (Time <= StunDuration - 30)
        {
            if (AttackTime > 0)
            {
                ...
                if (kbCounter < 10)
                {
                    float lerp = MathUtils.SineOutEasing(kbCounter / 10f);
                    NPC.rotation = AttackRotation.AngleLerp(-AttackRotation, lerp);
                }
                else
                {
                    float lerp = MathUtils.SineInOutEasing((kbCounter - 10) / 20f);
            ...
            {
                TeleportTime++;
                NPC.rotation = MathHelper.Lerp(AttackRotation, MathHelper.Pi / 24f * NPC.oldVelocity.X, MathUtils.CircOutEasing(MathHelper.Clamp(TeleportTime / 30f, 0f, 1f)));
            }
        }
        #endregion
        ...
        {
            float lerp = Time / 15f;
            ShieldOpacity = 1 - MathUtils.CircOutEasing(lerp);
        ...
        if (Time > StunDuration - 30)
        {
            NPC.rotation = NPC.rotation.AngleLerp(0f, MathUtils.SineInOutEasing((Time - (StunDuration - 30)) / 30f));
        ...
        if (Time >= StunDuration)
        {
            ...
            int spawnTime = GetBrainOfCthuluCreepersCount() / 2 * creeperRate;

            if (Time == StunDuration)
            {
                AttackCounter = GetBrainOfCthuluCreepersCount() - 1;
                SoundEngine.PlaySound(Roar, NPC.Center);
            }
            ...
            {
                ShieldOpacity = MathUtils.CircOutEasing(lerp);
                ShieldScale = MathHelper.Lerp(1.5f, 1f, MathUtils.SineOutEasing(lerp));
            }
            ...
            else if (AttackCounter > -1 && wrappedCounter == 0)
            {
                for (int i = 0; i < 2; i++)
                {
                    Vector2 dir = Vector2.UnitY.RotatedBy((AttackCounter % 2 == 0 ? 1 : -1) * (MathHelper.Pi / 16f + ((MathHelper.Pi - MathHelper.Pi / 8f) * (AttackCounter / 2f / (GetBrainOfCthuluCreepersCount() / 2f)))));
                    ...
                    for (int j = 0; j < 3; j++)
                    {
                        BloodDrop p = new(spawnPos, dir.RotatedBy(Main.rand.NextFloat(-MathHelper.Pi / 6f, MathHelper.Pi / 6f)) * Main.rand.NextFloat(8f, 12f), 32, 1f, Color.Red);
                        ParticleSystem.SpawnParticle(p);
                    }
                    BloodSplatter p2 = new(spawnPos, dir * 10f, 16, 0.5f, Color.Red);
    ...
    private void CreeperSwipes()
    {
        ...
        if (Time >= SwipesStartupDuration)
        {
            ...
            if (wrappedCount > 1 && wrappedCount <= SwipeIchorDelay && Time % 3 == 0) // Telegraphs ichor rain w/ dripping particles
            {
                ...
                BloodDrop blood = new BloodDrop(spawnPosition, Main.rand.NextVector2Unit() * Main.rand.NextFloat(1.5f, 2.5f), Main.rand.Next(30, 40), Main.rand.NextFloat(0.6f, 1f), Color.Gold);
                ParticleSystem.SpawnParticle(blood);
            }

            if (wrappedCount < 60f) // Vibrates during telegraph
            {
                Vector2 vibrationVector = Main.rand.NextVector2CircularEdge(1, 1) * MathHelper.Lerp(0f, 12f, MathUtils.CircInEasing(wrappedCount / 80f));
    ...
    private void CreeperOrbit()
    {
        ...
        if (Time == 0 && Main.netMode != NetmodeID.MultiplayerClient)
        {
            ...
            if (Main.netMode != NetmodeID.SinglePlayer)
            {
                ...
                int creepersPerExtraPlayer = Main.masterMode ? 3 : 2;
    ...
    private void CreeperSpiral()
    {
        ...
        if (Time > SpiralDuration - 30)
            timePerRev *= MathHelper.Lerp(1f, 10f, MathUtils.CircOutEasing(MathHelper.Clamp((Time - (SpiralDuration - 30)) / 30f, 0f, 1f)));
        else if (Time < SpiralSetupTime)
            timePerRev *= MathHelper.Lerp(1f, 10f, MathUtils.CircInEasing(MathHelper.Clamp(Time / SpiralSetupTime, 0f, 1f)));
        ...
        if (animCounter >= 0)
        {
            ...
            else if (animCounter == 65)
            {
                ...
                for (int j = 0; j < 20; j++)
                {
                    ...
                    BloodDrop b = new(NPC.Center - (Vector2.UnitY * 32) + (edgeBloodDir * new Vector2(16, 24)), edgeBloodDir.RotatedBy(Main.rand.NextFloat(-MathHelper.Pi / 10f, MathHelper.Pi / 10f)) * Main.rand.NextFloat(4f, 8f), 24, 0.75f, Color.Red);
                    ParticleSystem.SpawnParticle(b);
            ...
            if (animCounter < 60f)
                BoCDrawOffset = Main.rand.NextVector2CircularEdge(1, 1) * MathHelper.Lerp(0f, 16f, MathUtils.CircInEasing(animCounter / 60f));
            else if (animCounter < 70f)
                BoCDrawOffset = Main.rand.NextVector2CircularEdge(1, 1) * MathHelper.Lerp(16f, 0f, MathUtils.CircOutEasing((animCounter - 60) / 10f));
        ...
        else
        {
            ...
            if (Time < ChaseTime + (TeleportDuration / 2f))
            {
                if (Time % 4 == 0)
                {
                    ...
                    ParticleSystem.SpawnParticle(afterimage);
                }
                TeleportTime++;
            }
    ...
    private void Bloodletting()
    {
        ...
        if (endTime < 0)
        {
            ...
            DisableMultiplayerSmoothing = true;
            if (Time < 30f)
                NPC.Center = Vector2.Lerp(AttackPosition, goalPos, MathUtils.SineOutEasing(Time / 30f));
            else
                NPC.Center = goalPos;
        }
    ...
    private void SanguineScythes()
    {
        ...
        if (AttackCounter > SanguineTeleportCount)
        {
            ...
            if (Time > SanguineAttackEndDelay + SanguineAttackEndDuration + (Main.masterMode ? 20 : 40))
                SetupForNextAttack();
        }
        #endregion
        #region Attack Start
    ...
    private void CrimsonEyes()
    {
        ...
        if (Time <= 30)
        {
            if (Time >= 6 && Time <= 12)
            {
                if (Time == 6)
                {
                    ...
                    CameraSystem.StartScreenShake(NPC.Center, Vector2.Zero, 10f, 10, 60);
                }
        ...
        {
            #region Eye Spawning
            if (Time < CrimsonEyeAttackDuration && ProjectileUtils.CountProjectiles(ModContent.ProjectileType<CrimsonEye>()) < CrimsonEyeCap && Time % CrimsonEyeRate == 0)
            {
            ...
            else
            {
                ...
                if (Time >= CrimsonEyeAttackIdleDuration + CrimsonEyeAttackSetUpDuration && Time < CrimsonEyeAttackDuration)
                {
                    ...
                    {
                        float lerp = MathUtils.SineInEasing((Time - (CrimsonEyeAttackIdleDuration + CrimsonEyeAttackSetUpDuration)) / CrimsonEyeAttackBuildUpDuration);
    ...
    private void IllusionDash()
    {
        ...
        if (Time < IllusionDashTeleportDuration)
        {
            ...
            if (Time < (TeleportDuration / 2f))
            {
                if (Time % 4 == 0)
                {
                    ...
                    for (int i = 0; i < (Main.masterMode ? 8 : 4); i++)
                    {
                        Vector2 myEndPoint = AttackPosition + ((AttackRotation + (Main.masterMode ? MathHelper.PiOver4 : MathHelper.PiOver2) * i).ToRotationVector2() * IllusionDashTeleportDistance);
            ...
            else if (Time == (TeleportDuration / 2f) && !AttackFlag)
            {
                ...
                for (int i = 0; i < (Main.masterMode ? 8 : 4); i++)
                {
                    float rot = AttackRotation + (Main.masterMode ? MathHelper.PiOver4 : MathHelper.PiOver2) * i;
                ...
                if(Main.netMode != NetmodeID.SinglePlayer)
                {
                    ...
                    foreach (int who in targets)
                    {
                        ...
                        for (int i = 0; i < (Main.masterMode ? 4 : 2); i++)
                        {
                            float rot = baseRot + (Main.masterMode ? MathHelper.PiOver2 : MathHelper.Pi) * i;
        ...
        else
        {
            ...
            if (startTime == 0)
            {
                ...
                Sparkle sparkle = new(NPC.Center + new Vector2(16, -8), Vector2.Zero, Color.Yellow, Color.Orange, 2f, 16, needed: true);
            ...
            if (startTime < 30)
            {
                ...
                float circleDist = MathHelper.Lerp(IllusionDashTeleportDistance, IllusionDashCloseInDistance, MathUtils.SineOutEasing(lerp));
                NPC.Center = Vector2.Lerp(AttackPosition, Target.Center + (AttackRotation.ToRotationVector2() * circleDist), lerp);
                AttackRotation += MathHelper.Lerp(0f, IllusionDashStartingSpinSpeed, MathUtils.SineInEasing(lerp));
                DisableMultiplayerSmoothing = true;
            }
            else if (startTime <= 30 + IllusionDashSpinDuration)
            {
                ...
                DisableMultiplayerSmoothing = true;
                AttackRotation += MathHelper.Lerp(IllusionDashStartingSpinSpeed, 0f, MathUtils.SineOutEasing((startTime - 30) / (float)IllusionDashSpinDuration));
            }
            ...
            else if (startTime <= 150 + IllusionDashSpinDuration + 30 + IllusionDashFakeoutTeleportDuration + 30)
            {
                ...
                if (startTime % 15 == 5 && startTime > 60 + IllusionDashSpinDuration + 30 + IllusionDashFakeoutTeleportDuration)
                {
                    ...
                    for (int i = 0; i < (Main.masterMode ? 2 : 1); i++)
                    {
                        if (Main.netMode != NetmodeID.MultiplayerClient)
                        {
        ...
        if (Time >= 90)
        {
            if (Time == 90)
            {
                foreach (Projectile p in Main.ActiveProjectiles)
                {
                    ...
                    p.GetGlobalProjectile<BrainOfCthulhuGlobalProjectile>().IgnoreBoCIllusions = true;
                }
            ...
            else if (Time >= IllusionTrickTimeLimit) //Players have failed to find the real BoC within the time limit
            {
                if (Time == IllusionTrickTimeLimit)
                {
                    ...
                    foreach (NPC n in Main.ActiveNPCs)
                    {
                        ...
                        (n.ModNPC as FalseBrain).BeenHit = true;
                        n.netUpdate = true;
                    }
            ...
            else
            {
                ...
                float lerp = MathUtils.SineInOutEasing(MathHelper.Clamp((Time - 150) / 30f, 0f, 1f));
                float baseDist = 240;
    ...
    private void DeathAnimation()
    {
        ...
        for (int i = 0; i < bloodGushingData.Length; i++)
        {
            if (Time >= bloodGushingData[i].time)
            {
                if (Time == bloodGushingData[i].time)
                {
                    ...
                    BloodSplatter p2 = new(NPC.Center + bloodGushingData[i].offset.RotatedBy(NPC.rotation), bloodDir * 7.5f, 16, 0.5f, Color.Red);
                    ParticleSystem.SpawnParticle(p2);
                    ...
                    CameraSystem.StartScreenShake(NPC.Center, Vector2.Zero, 1f, 10, 60);
                }

                for (int j = 0; j < 2; j++)
                {
                    BloodDrop p = new(NPC.Center + bloodGushingData[i].offset.RotatedBy(NPC.rotation), (bloodGushingData[i].angle + Main.rand.NextFloat(-MathHelper.Pi / 10f, MathHelper.Pi / 10f)).ToRotationVector2() * Main.rand.NextFloat(5f, 10f), 32, 1f, Color.Red);
                    ParticleSystem.SpawnParticle(p);
                }
            }
        ...
        if (Time >= EndTime)
        {
            ...
            CameraSystem.StartScreenShake(NPC.Center, Vector2.Zero, 2f, 10, 60);
            ...
            for (int i = 0; i < pCount; i++)
            {
                ...
                for (int j = 0; j < 2; j++)
                {
                    BloodDrop p = new(NPC.Center, pVelo.RotatedBy(Main.rand.NextFloat(-MathHelper.Pi / 6f, MathHelper.Pi / 6f)) * Main.rand.NextFloat(0.5f, 1f), 32, 1f, Color.Red);
                    ParticleSystem.SpawnParticle(p);
                }
                BloodSplatter p2 = new(NPC.Center, pVelo * 0.75f, 16, 0.5f, Color.Red);
                ParticleSystem.SpawnParticle(p2);
            }
        ...
        {
            Vector2 edgeBloodDir = Main.rand.NextVector2CircularEdge(1, 1);
            BloodDrop b = new(NPC.Center + (edgeBloodDir * NPC.Size * 0.75f), edgeBloodDir.RotatedBy(Main.rand.NextFloat(-MathHelper.Pi / 10f, MathHelper.Pi / 10f)) * Main.rand.NextFloat(2f, 4f), 16, 0.75f, Color.Red);
            ParticleSystem.SpawnParticle(b);
        }
    }
    ...
    public override void SendExtraAI(BitWriter bitWriter, BinaryWriter binaryWriter)
    {
        ...
        if (AIState <= BrainAIState.SurfaceSpawnAnimation)
        {
            ...
            binaryWriter.Write(WasSummonedViaItem);
        }
    ...
    public override void ReceiveExtraAI(BitReader bitReader, BinaryReader binaryReader)
    {
        ...
        if (AIState <= BrainAIState.SurfaceSpawnAnimation)
        {
            ...
            WasSummonedViaItem = binaryReader.ReadBoolean();
        }
    ...
    {
        if (AIState == BrainAIState.IllusionTrick && !AttackFlag && projectile.GetGlobalProjectile<BrainOfCthulhuGlobalProjectile>().IgnoreBoCIllusions)
            return false;
    ...
    public override bool PreDraw(Mod mod, SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
    {
        ...
        if (phase1)
        {
            ...
            {
                VerletObject curvePoints = v.tendril;
                ...
                for (int i = 0; i < curvePoints.Count; i++)
                {
                    ...
                    if (creeper != null)
                    {
                        ...
                        ichorRatio = MathUtils.ExpInEasing((float)Math.Sin((flowTime + (i * flowAmt)) * 2f) / 2f + 0.5f);
                    }
    ...
    public static int GetBrainOfCthuluCreepersCount() => Main.masterMode ? 27 : 18;
    ...
    private void SelectNewTarget()
    {
        ...
        options.finishThemOff = true;
        options.maxSearchRange = DespawnRange;
        options.targetType = TargetingParameters.NPCTargetType.ForceSwitch;
    ...
    private static void DrawBrain(SpriteBatch spriteBatch, NPC brain)
    {
        ...
        if (!phase1 && ai.TeleportTime != 0)
        {
            ...
            scale = Vector2.Lerp(Vector2.One, new Vector2(0.5f + ((float)Math.Cos(ai.Time / (ai.TeleportDuration / 2f) * MathHelper.TwoPi) / 2f + 0.5f), 0.5f + ((float)Math.Sin(ai.Time / (ai.TeleportDuration / 2f) * MathHelper.TwoPi) / 2f + 0.5f)), MathUtils.SineInOutEasing(ai.TeleportTime / (ai.TeleportDuration / 2f)));
