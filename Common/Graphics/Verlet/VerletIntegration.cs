...
using System.Linq;
using Terraria;
using static BigEvil.Common.Graphics.Verlet.VerletIntegration;

namespace BigEvil.Common.Graphics.Verlet;
            ...
            if (isChain)
            {
                ...
                for (int j = 0; j < segment.Connections.Count; j++)
                {
                    ...
                    if (Collision.CheckAABBvLineCollision(e.TopLeft, e.Size, segment.Position, next.Position, 8f, ref _))
                    {
                        ...
                        float currentMovementOffsetInterpolant = Terraria.Utils.GetLerpValue(distanceToEntity, distanceBetweenSegments, distanceBetweenSegments * 0.2f, true);
